...
impl Compiler {
  ...
  pub(crate) fn is_color(&self) -> bool {
    ...
    while let Some(Objects::Char(ch)) = self.get(self.index + offset) {
      if *ch == ' ' {
        break;
      }
      string.push(*ch);
  ...
  pub(crate) fn peak(&self, str_: &[char]) -> bool {
    ...
    loop {
      ...
      if str_.get(i).unwrap().to_ascii_lowercase() != cha.to_ascii_lowercase() {
        return false;
      }
      i += 1;
    }
...
pub static TABLEOFCONTENTS: [char; 16] = [
